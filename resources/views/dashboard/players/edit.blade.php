    ...
    <div class="row justify-content-center">
        <div class="col-md-10">
                ...
                <div class="card card-primary">
                  ...
                  <!-- /.card-header -->
                  <!-- form start -->
                  {!! Form::open(['action' => ['PlayerController@update', $data['player']->id], 'method' => 'POST', 'enctype' => 'multipart/form-data']) !!}
                    <div class="card-body">
                      <div class="form-group">
                        {{Form::label('name',  'Player Name')}}
                        {{Form::text('name', $data['player']->name, ['class' => 'form-control', 'placeholder' => 'Player Name'])}}
                      </div>
                        <div class="form-group">
                            {{Form::label('job', 'Job')}}
                            {{Form::text('job', $data['player']->job, ['class' => 'form-control', 'placeholder' => 'Place in the Team'])}}
                            </div>
                        <div class="form-group">
                            {{Form::label('player_phone_number', 'Player Phone Number')}}
                            {{Form::number('player_phone_number', $data['player']->player_phone_number, ['class' => 'form-control', 'placeholder' => ''])}}
                         </div>
                         <div class="form-group">
                            {{Form::label('player_email', 'Player Email Address')}}
                            {{Form::email('player_email', $data['player']->player_email, ['class' => 'form-control', 'placeholder' => 'Email'])}}
                        </div>
                        <div class="form-group">
                            {{Form::label('player_address', 'Player Address')}}
                            {{Form::textarea('player_address', $data['player']->player_address, ['id' => 'article-ckeditor', 'class' => 'form-control', 'placeholder' => 'Write Something About Your Self'])}}
                      </div>
                      <div class="form-group">
